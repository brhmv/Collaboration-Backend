...
import static az.edu.turing.turingcollab.model.constant.AppConstant.DATE_FORMAT_4;
...
public interface MentoriumMapper {
    ...
    @Mapping(source = "request.startTime", target = "startTime", dateFormat = DATE_FORMAT_4)
    @Mapping(source = "request.endTime", target = "endTime", dateFormat = DATE_FORMAT_4)
    @Mapping(target = "imageName", ignore = true)
    MentoriumEntity toEntity(Long createdBy, MentoriumCreateRequest request);

    @Mapping(source = "mentorium.id", target = "id")
    @Mapping(source = "mentorium.topic", target = "topic")
    @Mapping(source = "mentorium.imageName", target = "imageName")
    @Mapping(source = "mentorium", target = "creator")
    MentoriumCardResponse toCardResponse(MentoriumEntity mentorium, UserEntity creator);
